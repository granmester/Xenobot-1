...
------------------------------------------------------------------------------------------------------------------
	...
	_Remote_AdminName = { "Szwagru", "Druid Angel" }									-- Name of the character used to request status information
	_Remote_OutcomeCommand = "outcome?"								-- Command to send outcome information
	_Remote_CashCommand = "cash?"						-- Command to send available cash information
	_Remote_ItemsCommand = "items?"						-- Command to send how much the items in the container are worth
...
------------------------------------------------------------------------------------------------------------------
	...
	Pair_Of_Numbers_Payout = 120
	
	-- Single Numbers = 1, 2, 3, 4, 5 or 6 --
	Single_Numbers = true
	Single_Numbers_Minimum = 30000
	Single_Numbers_Maximum = 300000
	Single_Numbers_Payout = 180
				...
				if Pay_Cash_Signal ~= 0 then
					...
					if Roll_Dice_Signal then
						...
						if (_VirtualMode_) then
							Dice_Rolled_Check = 1
							local randommode = math.random(1, 120)
							local randix = nil
								if randommode >= 1 and randommode <= 20 then
									randix = 1
								elseif randommode >= 21 and randommode <= 40 then
									randix = 2
								elseif randommode >= 41 and randommode <= 60 then
									randix = 3
								elseif randommode >= 61 and randommode <= 80 then
									randix = 4
								elseif randommode >= 81 and randommode <= 100 then
									randix = 5
								elseif randommode >= 101 and randommode <= 120 then
									randix = 6
								end
							ProcessDebugMessage('Casino Debugger', 'wylosowano ' .. randommode .. ' czyli ' .. randix .. '.')
							EffectProxyMessage = Self.Name()..' rolled a '.. randix ..'.'
						else
